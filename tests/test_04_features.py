...
Page move/rename tests: test_05_page_move.py
Redirect (#REDIRECT) tests: test_06_redirects.py
"""
# -----------------------------------------------------------------------------
...
async def test_special_pages_shows_categories(client, db_session):
    ...
    resp = await client.get("/special")
    assert resp.status_code == 200
    assert "/special/categories" in resp.text
    assert "category currently in use" in resp.text


@pytest.mark.asyncio
async def test_special_categories_page(client, db_session):
    headers = await _setup(client, db_session, "spuser4", "SPNS4")
    await _create_page(client, "SPNS4", "Cat A", "[[Category:Alpha]]", "markdown", headers)
    await _create_page(client, "SPNS4", "Cat B", "[[Category:Beta]]", "markdown", headers)
    await _create_page(client, "SPNS4", "Cat A2", "[[Category:Alpha]]", "markdown", headers)

    resp = await client.get("/special/categories")
    assert resp.status_code == 200
    html = resp.text
    assert "Alpha" in html
    assert "Beta" in html
    assert "2 members" in html
    assert "1 member" in html


@pytest.mark.asyncio
async def test_special_categories_filter(client, db_session):
    headers = await _setup(client, db_session, "spuser5", "SPNS5")
    await _create_page(client, "SPNS5", "P1", "[[Category:Aardvark]]", "markdown", headers)
    await _create_page(client, "SPNS5", "P2", "[[Category:Zebra]]", "markdown", headers)

    resp = await client.get("/special/categories?from_=A")
    assert resp.status_code == 200
    assert "Aardvark" in resp.text
    assert "Zebra" not in resp.text
